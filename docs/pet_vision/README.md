...
For the approved deployed producer, after integration/QA accepts the continuous entry point:

```powershell
# PET_VISION_TOKEN is loaded locally by the launcher, never passed on a command line.
& .runtime/vision-venv/Scripts/python.exe tools/run_pet_vision.py --provider pet_vision.ipc:leased_opencv_frames --continuous --send --event-log .runtime/vision-events.ndjson
```

This is explicit opt-in; bounded runs remain the default. Continuous mode is available only for DirectShow. A Windows named mutex rejects a second pet-vision camera reader before opening a device. The child exits after 20 seconds without reader progress; the parent terminates its own reader tree after 30 seconds without a complete frame. Official media reacquisition, daemon error, camera read error or HTTP delivery error stops the runner with no auto-restart. Stop a foreground run with Ctrl+C; for a hidden launch retain its root PID and use `taskkill /PID <root-pid> /T /F` to stop only that producer tree. Do not kill other Python/daemon/audio processes. Metadata logs rotate at 1 MiB with one backup; no pixel, waveform, token or biometric data is written. The HTTP receipt is included in event metadata, and an accepted receipt still does not imply action completion.
...
- Subsequent authorized 60-second real-frame deployment test: 264 distinct frames in 60.36 seconds, two slow reads dropped, 66 real no-hand events delivered to port 8091 and all accepted. Backend hand_visibility was not_visible during capture and unknown after producer exit/lease expiry. Speech remained connected/listening, error null, capture counter increased and dropped_frames stayed zero. No fake hand events were sent and the motion backend remained dryrun. Continuous-entry tests additionally cover singleton exclusion, release, stalled progress and invalid continuous CLI combinations (25 focused tests passed).
