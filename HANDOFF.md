...
Kim明确要求本项目所有领域任务对她可见。侧边栏已建立“带上她的眼睛”分组，9个任务职责和完整ID见 PROJECT_CONTROL.md。视频独立任务已接手待改的9镜；采集、服务、QA已完成首轮只读接手。原M5总控/固件/硬件/美术沿用现有任务和唯一权限，不新建重复固件owner。后续不要用不可见内部子代理替代可见领域任务。

## 外接震动候选评审

- 已读取 reading-pet 固定版本 6068425bccee72caf908434df36f5e8373b40dfd 的 docs/hardware/HAPTICS_FEASIBILITY.md；PRD 已引用权威链接及实物放行条件。
- DRV2605L #2305 + #1201 ERM + 独立3.3V稳压列入 MVP 候选，仍未采购/接线/实测；不能承诺直接插接、塞入原壳或触感已验。硬件负责人维护接线与参数，haifeng 不复制驱动代码或接线表。
- 仅首次呈现新回应提醒，快照/重看/重试/重连不补震，故障不阻塞显示。此次仅文档评审和同步，不启动实施。
...
## Reachy 启动修复：原生路径与隔离副本
- 2026-09-23 22:48 已给原生 .venv/apps_venv 补齐 guard，官方入口重试后 daemon 可运行；细节及限制见 patches/reachy_expression/README.md 最后一节。
